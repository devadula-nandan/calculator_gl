...
const performOperation = (num1, num2) => {
	...
	if (_result.includes(".")){
		if (_result.split('.')[0].length <= 8) return _result.split('.')[0] + "." + (_result.split('.')[1].length > 8 - _result.split('.')[0].length  ? (_result.split('.')[1][0 , (8-_result.split('.')[0].length])) :(_result.split('.')[1]) )
